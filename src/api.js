...
export const getEvents = async () => {
  ...
  if (token) {
    ...
    const url = `https://3l3qgkp959.execute-api.us-east-2.amazonaws.com/dev/api/get-events/${token}`;
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://3l3qgkp959.execute-api.us-east-1.amazonaws.com/dev/api/get-auth-url"
      );
...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    `https://3l3qgkp959.execute-api.us-east-1.amazonaws.com/dev/api/token/${encodeCode}`
  )
    .then((res) => {
      return res.json();
    })
